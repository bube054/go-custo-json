...
func TestLexString(t *testing.T) {
	var tests = []LexerTest{
		...
		{msg: "Lex valid simple single quote string with AllowLineComments", input: []byte(`'This is a single quote string'`), expected: []Token{NewToken(STRING, QUOTED, []byte(`'This is a single quote string'`), 1, 0, nil), NewToken(EOF, NONE, nil, 1, 31, nil)}, cfg: NewConfig(WithAllowSingleQuotes(true))},
		{msg: "Lex empty single string", input: []byte(`''`), expected: []Token{NewToken(STRING, QUOTED, []byte(`''`), 1, 0, nil), NewToken(EOF, NONE, nil, 1, 2, nil)}, cfg: NewConfig(WithAllowSingleQuotes(true))},
		{msg: "Lex non-empty single string", input: []byte(`'text'`), expected: []Token{NewToken(STRING, QUOTED, []byte(`'text'`), 1, 0, nil), NewToken(EOF, NONE, nil, 1, 6, nil)}, cfg: NewConfig(WithAllowSingleQuotes(true))},
		{msg: "Lex esc single string", input: []byte(`'\''`), expected: []Token{NewToken(STRING, QUOTED, []byte(`'\''`), 1, 0, nil), NewToken(EOF, NONE, nil, 1, 4, nil)}, cfg: NewConfig(WithAllowSingleQuotes(true))},
		{msg: "Lex esc backward slash single string", input: []byte(`'\\'`), expected: []Token{NewToken(STRING, QUOTED, []byte(`'\\'`), 1, 0, nil), NewToken(EOF, NONE, nil, 1, 4, nil)}, cfg: NewConfig(WithAllowSingleQuotes(true))},
		...
		{msg: "Lex esc forward slash single string", input: []byte(`'\/'`), expected: []Token{NewToken(STRING, QUOTED, []byte(`'\/'`), 1, 0, nil), NewToken(EOF, NONE, nil, 1, 4, nil)}, cfg: NewConfig(WithAllowSingleQuotes(true))},
		{msg: "Lex esc backspace single string", input: []byte(`'\b'`), expected: []Token{NewToken(STRING, QUOTED, []byte(`'\b'`), 1, 0, nil), NewToken(EOF, NONE, nil, 1, 4, nil)}, cfg: NewConfig(WithAllowSingleQuotes(true))},
		{msg: "Lex esc form feed single string", input: []byte(`'\f'`), expected: []Token{NewToken(STRING, QUOTED, []byte(`'\f'`), 1, 0, nil), NewToken(EOF, NONE, nil, 1, 4, nil)}, cfg: NewConfig(WithAllowSingleQuotes(true))},
		{msg: "Lex esc new line single string", input: []byte(`'\n'`), expected: []Token{NewToken(STRING, QUOTED, []byte(`'\n'`), 1, 0, nil), NewToken(EOF, NONE, nil, 1, 4, nil)}, cfg: NewConfig(WithAllowSingleQuotes(true))},
		{msg: "Lex esc carriage return single string", input: []byte(`'\r'`), expected: []Token{NewToken(STRING, QUOTED, []byte(`'\r'`), 1, 0, nil), NewToken(EOF, NONE, nil, 1, 4, nil)}, cfg: NewConfig(WithAllowSingleQuotes(true))},
		{msg: "Lex esc tab single string", input: []byte(`'\t'`), expected: []Token{NewToken(STRING, QUOTED, []byte(`'\t'`), 1, 0, nil), NewToken(EOF, NONE, nil, 1, 4, nil)}, cfg: NewConfig(WithAllowSingleQuotes(true))},
		{msg: "Lex esc 4 hex digits single string", input: []byte(`'\u597D'`), expected: []Token{NewToken(STRING, QUOTED, []byte(`'\u597D'`), 1, 0, nil), NewToken(EOF, NONE, nil, 1, 8, nil)}, cfg: NewConfig(WithAllowSingleQuotes(true))},
		...
		{msg: "Lex invalid escape single string with AllowEscapeChars", input: []byte(`'\Users'`), expected: []Token{NewToken(STRING, QUOTED, []byte(`'\Users'`), 1, 0, nil), NewToken(EOF, NONE, nil, 1, 8, nil)}, cfg: NewConfig(WithAllowSingleQuotes(true), WithAllowOtherEscapeChars(true))},
		...
		{msg: "Lex valid double single quote string with AllowLineComments", input: []byte(`"This is a double quote string"`), expected: []Token{NewToken(STRING, QUOTED, []byte(`"This is a double quote string"`), 1, 0, nil), NewToken(EOF, NONE, nil, 1, 31, nil)}, cfg: NewConfig(WithAllowSingleQuotes(true))},
		{msg: "Lex empty double string", input: []byte(`""`), expected: []Token{NewToken(STRING, QUOTED, []byte(`""`), 1, 0, nil), NewToken(EOF, NONE, nil, 1, 2, nil)}, cfg: NewConfig(WithAllowSingleQuotes(true))},
		{msg: "Lex non-empty double string", input: []byte(`"text"`), expected: []Token{NewToken(STRING, QUOTED, []byte(`"text"`), 1, 0, nil), NewToken(EOF, NONE, nil, 1, 6, nil)}, cfg: NewConfig(WithAllowSingleQuotes(true))},
		{msg: "Lex esc double string", input: []byte(`"\""`), expected: []Token{NewToken(STRING, QUOTED, []byte(`"\""`), 1, 0, nil), NewToken(EOF, NONE, nil, 1, 4, nil)}, cfg: NewConfig(WithAllowSingleQuotes(true))},
		{msg: "Lex esc backward slash double string", input: []byte(`"\\"`), expected: []Token{NewToken(STRING, QUOTED, []byte(`"\\"`), 1, 0, nil), NewToken(EOF, NONE, nil, 1, 4, nil)}, cfg: NewConfig(WithAllowSingleQuotes(true))},
		...
		{msg: "Lex esc forward slash double string", input: []byte(`"\/"`), expected: []Token{NewToken(STRING, QUOTED, []byte(`"\/"`), 1, 0, nil), NewToken(EOF, NONE, nil, 1, 4, nil)}, cfg: NewConfig(WithAllowSingleQuotes(true))},
		{msg: "Lex esc backspace double string", input: []byte(`"\b"`), expected: []Token{NewToken(STRING, QUOTED, []byte(`"\b"`), 1, 0, nil), NewToken(EOF, NONE, nil, 1, 4, nil)}, cfg: NewConfig(WithAllowSingleQuotes(true))},
		{msg: "Lex esc form feed double string", input: []byte(`"\f"`), expected: []Token{NewToken(STRING, QUOTED, []byte(`"\f"`), 1, 0, nil), NewToken(EOF, NONE, nil, 1, 4, nil)}, cfg: NewConfig(WithAllowSingleQuotes(true))},
		{msg: "Lex esc new line double string", input: []byte(`"\n"`), expected: []Token{NewToken(STRING, QUOTED, []byte(`"\n"`), 1, 0, nil), NewToken(EOF, NONE, nil, 1, 4, nil)}, cfg: NewConfig(WithAllowSingleQuotes(true))},
		{msg: "Lex esc carriage return double string", input: []byte(`"\r"`), expected: []Token{NewToken(STRING, QUOTED, []byte(`"\r"`), 1, 0, nil), NewToken(EOF, NONE, nil, 1, 4, nil)}, cfg: NewConfig(WithAllowSingleQuotes(true))},
		{msg: "Lex esc tab double string", input: []byte(`"\t"`), expected: []Token{NewToken(STRING, QUOTED, []byte(`"\t"`), 1, 0, nil), NewToken(EOF, NONE, nil, 1, 4, nil)}, cfg: NewConfig(WithAllowSingleQuotes(true))},
		{msg: "Lex esc 4 hex digits double string", input: []byte(`"\u4F60"`), expected: []Token{NewToken(STRING, QUOTED, []byte(`"\u4F60"`), 1, 0, nil), NewToken(EOF, NONE, nil, 1, 8, nil)}, cfg: NewConfig(WithAllowSingleQuotes(true))},
		...
		{msg: "Lex invalid escape double string with AllowEscapeChars", input: []byte(`"\Users"`), expected: []Token{NewToken(STRING, QUOTED, []byte(`"\Users"`), 1, 0, nil), NewToken(EOF, NONE, nil, 1, 8, nil)}, cfg: NewConfig(WithAllowSingleQuotes(true), WithAllowOtherEscapeChars(true))},
